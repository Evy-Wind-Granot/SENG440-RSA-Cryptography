...
mmm_step:
	...
	str	r2, [r7, #4]	@ Y0, Y0
	str	r3, [r7]	@ Y, Y
@ V3_loop_unrolling.c:8:     unsigned int T0  = T & 1U;
	ldr	r3, [r7, #12]	@ tmp122, T
	and	r3, r3, #1	@ T0_7, tmp122,
	str	r3, [r7, #28]	@ T0_7, T0
@ V3_loop_unrolling.c:9:     unsigned int eta = T0 ^ (Xi & Y0);
...
MMM:
	...
	str	r2, [r7, #4]	@ M, M
	str	r3, [r7]	@ m, m
@ V3_loop_unrolling.c:17:     unsigned int T = 0;
	movs	r3, #0	@ tmp136,
	str	r3, [r7, #44]	@ tmp136, T
@ V3_loop_unrolling.c:18:     unsigned int UX = (unsigned int)X;
	ldr	r3, [r7, #12]	@ tmp137, X
	str	r3, [r7, #36]	@ tmp137, UX
@ V3_loop_unrolling.c:19:     unsigned int UY = (unsigned int)Y;
	ldr	r3, [r7, #8]	@ tmp138, Y
	str	r3, [r7, #32]	@ tmp138, UY
@ V3_loop_unrolling.c:20:     unsigned int UM = (unsigned int)M;
	ldr	r3, [r7, #4]	@ tmp139, M
	str	r3, [r7, #28]	@ tmp139, UM
@ V3_loop_unrolling.c:21:     unsigned int Y0 = UY & 1U;
	ldr	r3, [r7, #32]	@ tmp141, UY
	and	r3, r3, #1	@ Y0_34, tmp141,
	str	r3, [r7, #24]	@ Y0_34, Y0
@ V3_loop_unrolling.c:24:     int i = 0;
	movs	r3, #0	@ tmp142,
	str	r3, [r7, #40]	@ tmp142, i
@ V3_loop_unrolling.c:25:     int limit = m - (m % 4);
	ldr	r3, [r7]	@ tmp143, m
...
@ V3_loop_unrolling.c:26:     for (; i < limit; i += 4) {
	b	.L4		@
.L5:
@ V3_loop_unrolling.c:27:         T = mmm_step(T, (UX >> i)       & 1U, Y0, UY, UM);
...
@ V3_loop_unrolling.c:27:         T = mmm_step(T, (UX >> i)       & 1U, Y0, UY, UM);
	...
	bl	mmm_step(PLT)	@
	str	r0, [r7, #44]	@, T
@ V3_loop_unrolling.c:28:         T = mmm_step(T, (UX >> (i + 1)) & 1U, Y0, UY, UM);
...
@ V3_loop_unrolling.c:28:         T = mmm_step(T, (UX >> (i + 1)) & 1U, Y0, UY, UM);
	...
	bl	mmm_step(PLT)	@
	str	r0, [r7, #44]	@, T
@ V3_loop_unrolling.c:29:         T = mmm_step(T, (UX >> (i + 2)) & 1U, Y0, UY, UM);
...
@ V3_loop_unrolling.c:29:         T = mmm_step(T, (UX >> (i + 2)) & 1U, Y0, UY, UM);
	...
	bl	mmm_step(PLT)	@
	str	r0, [r7, #44]	@, T
@ V3_loop_unrolling.c:30:         T = mmm_step(T, (UX >> (i + 3)) & 1U, Y0, UY, UM);
...
@ V3_loop_unrolling.c:26:     for (; i < limit; i += 4) {
	...
	blt	.L5		@,
@ V3_loop_unrolling.c:32:     for (; i < m; i++) {
	b	.L6		@
.L7:
@ V3_loop_unrolling.c:33:         T = mmm_step(T, (UX >> i) & 1U, Y0, UY, UM);
...
@ V3_loop_unrolling.c:32:     for (; i < m; i++) {
	...
	blt	.L7		@,
@ V3_loop_unrolling.c:36:     maskGE = 0U - (unsigned int)(T >= UM);
...
@ V3_loop_unrolling.c:37:     T -= maskGE & UM;
...
@ V3_loop_unrolling.c:37:     T -= maskGE & UM;
	...
	str	r3, [r7, #44]	@ T_40, T
@ V3_loop_unrolling.c:38:     maskGE = 0U - (unsigned int)(T >= UM);
...
@ V3_loop_unrolling.c:39:     T -= maskGE & UM;
...
@ V3_loop_unrolling.c:39:     T -= maskGE & UM;
	...
	str	r3, [r7, #44]	@ T_42, T
@ V3_loop_unrolling.c:41:     return (int)T;
	ldr	r3, [r7, #44]	@ _43, T
@ V3_loop_unrolling.c:42: }
	mov	r0, r3	@, <retval>
	adds	r7, r7, #48	@,,
	mov	sp, r7	@,
	@ sp needed	@
...
main:
	...
	sub	sp, sp, #88	@,,
	add	r7, sp, #16	@,,
@ V3_loop_unrolling.c:46:     const int M = 3233;
	movw	r3, #3233	@ tmp139,
	str	r3, [r7, #64]	@ tmp139, M
@ V3_loop_unrolling.c:47:     const int m = 12;
	movs	r3, #12	@ tmp140,
	str	r3, [r7, #60]	@ tmp140, m
@ V3_loop_unrolling.c:48:     const long N = 20000000L;
	mov	r3, #11520	@ tmp141,
	movt	r3, 305	@ tmp141,
	str	r3, [r7, #56]	@ tmp141, N
@ V3_loop_unrolling.c:49:     volatile long acc = 0;
	movs	r3, #0	@ tmp142,
	str	r3, [r7, #36]	@ tmp142, acc
@ V3_loop_unrolling.c:52:     clock_gettime(CLOCK_MONOTONIC, &t0);
	add	r3, r7, #16	@ tmp143,,
	mov	r1, r3	@, tmp143
	movs	r0, #1	@,
	bl	__clock_gettime64(PLT)	@
@ V3_loop_unrolling.c:54:     for (long i = 0; i < N; i++) {
	movs	r3, #0	@ tmp144,
	str	r3, [r7, #68]	@ tmp144, i
@ V3_loop_unrolling.c:54:     for (long i = 0; i < N; i++) {
	b	.L10		@
.L11:
@ V3_loop_unrolling.c:55:         int X = (int)((i * 2654435761U) % 4096U);
...
@ V3_loop_unrolling.c:55:         int X = (int)((i * 2654435761U) % 4096U);
	...
	str	r3, [r7, #44]	@ X_38, X
@ V3_loop_unrolling.c:56:         int Y = (int)((i * 40503U + 1) % 4096U);
...
@ V3_loop_unrolling.c:56:         int Y = (int)((i * 40503U + 1) % 4096U);
	...
	str	r3, [r7, #40]	@ Y_39, Y
@ V3_loop_unrolling.c:57:         acc += MMM(X, Y, M, m);
	ldr	r3, [r7, #60]	@, m
	ldr	r2, [r7, #64]	@, M
	ldr	r1, [r7, #40]	@, Y
	ldr	r0, [r7, #44]	@, X
...
@ V3_loop_unrolling.c:54:     for (long i = 0; i < N; i++) {
	...
	blt	.L11		@,
@ V3_loop_unrolling.c:60:     clock_gettime(CLOCK_MONOTONIC, &t1);
	mov	r3, r7	@ tmp153,
	mov	r1, r3	@, tmp153
	movs	r0, #1	@,
	bl	__clock_gettime64(PLT)	@
@ V3_loop_unrolling.c:61:     double secs = (t1.tv_sec - t0.tv_sec) + (t1.tv_nsec - t0.tv_nsec) / 1e9;
...
@ V3_loop_unrolling.c:64:            N, secs, secs * 1e9 / N, acc);
	vldr.64	d7, [r7, #48]	@ tmp156, secs
	vldr.64	d6, .L13	@ tmp157,
	vmul.f64	d5, d7, d6	@ _19, tmp156, tmp157
@ V3_loop_unrolling.c:63:     printf("v3_unroll: %ld MMM calls in %.4f s  (%.1f ns/call)  acc=%ld\n",
...
.LPIC0:
	add	r0, pc	@ tmp159
	bl	printf(PLT)	@
@ V3_loop_unrolling.c:65:     return 0;
	movs	r3, #0	@ _35,
@ V3_loop_unrolling.c:66: }
	mov	r0, r3	@, <retval>
	adds	r7, r7, #72	@,,
	mov	sp, r7	@,
	@ sp needed	@
